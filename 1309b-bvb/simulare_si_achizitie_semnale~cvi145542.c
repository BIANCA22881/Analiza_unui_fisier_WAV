...
int CVICALLBACK OnLoadButtonCB (int panel, int control, int event,void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			...
			buffer2= (double *) malloc(16284* sizeof(double));			
						...
						void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_TIMER_TICK:
			...
			if (StartSpect > 5)
				StartSpect=0;
			int sec=StartSpect*fs;
			for(int i=sec;i<sec+N;i++)
			{
				buffer[i-sec]=waveData[i];
			}
			//spectru
			ScaledWindowEx (buffer, N, RECTANGLE, 0, &winConst);
			AutoPowerSpectrum(buffer,N,1.0/fs,autoSpectrum,&df);
			...
			PlotWaveform (acqPanel, PANEL_2_GRAPH, spectru, N/2, VAL_DOUBLE, 1.0, 0.0, 0.0, 1.0, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_RED);
			//afisare raw data pe al doilea panel
			DeleteGraphPlot (acqPanel, PANEL_2_GRAPH_2, -1, VAL_IMMEDIATE_DRAW);
			PlotY (acqPanel, PANEL_2_GRAPH_2,waveData ,npoints , VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_RED);
			/*
			filtru
			ferestruiere
			Spectru
			*/
			...
			{//atunci cand apas pe start
				...
				//downsampling
				double *temporar = (double*)malloc((npoints/4)*sizeof(double));
				for(int i=0;i<npoints;i+=4)
				{ 
					temporar[i/4]=(waveData[i]+waveData[i+1]+waveData[i+2]+waveData[i+3])/4;
				}
				if(tipFiltru == 0)
				{
					Bw_HPF (waveData, npoints,sampleRate ,1/3*frequencyPeak , 5, dataFiltrata);
					//afisare data filtrata
					DeleteGraphPlot (acqPanel, PANEL_2_GRAPH_3, -1, VAL_IMMEDIATE_DRAW);
					PlotY (acqPanel, PANEL_2_GRAPH_3,dataFiltrata ,npoints , VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_RED);
				...
				{
					printf(" %lf %d ",frequencyPeak,fs/8);
					Ch_BPF (waveData, npoints, sampleRate, (1/4)*frequencyPeak, (1/2)*frequencyPeak, 0.1, 5, dataFiltrata);
					//afisare data filtrata
					DeleteGraphPlot (acqPanel, PANEL_2_GRAPH_3, -1, VAL_IMMEDIATE_DRAW);
					PlotY (acqPanel, PANEL_2_GRAPH_3,dataFiltrata ,npoints , VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_RED);
									   ...
									   void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			...
			{//atunci cand apas pe start
				DeleteGraphPlot (acqPanel, PANEL_2_GRAPH_3, -1, VAL_IMMEDIATE_DRAW);
				PlotY (acqPanel, PANEL_2_GRAPH_3, waveData, npoints, VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_RED);
			}
			else
			{//atunci cand apas pe stop
				DeleteGraphPlot (acqPanel, PANEL_2_GRAPH_3, -1, VAL_IMMEDIATE_DRAW);
...
bwHPs,invCH
nu este deloc riplu
chebishev 2 are riplu la final 

bwcoef
a = vectori coeficienti
b = vector coeficient numarator

CxPolyRootsEx -> Rad complexe / zerouri +poli
ToPolar1D() reprezinta pe cercul unitate


frevventa aproape de zerourui rexulta frecventa atenutata
frecventa aproape de pol rezulta frecventa amplificata 
zero pe cercul unitate rezulta frecventa anulata
*/
...
/*GetCtrlVal(acqPanel,PANEL_2_TOGGLEBUTTON_2,&doSpectruF);
			if(doSpectruF == 1)
			{
				...
				if(tipFiltru == 0)
				{
					...
					if(tipFereastra == 0)
					{
						for(int i=sec;i<sec+N;i++)
						{
							buffer2[i-sec]=waveData[i];
						}
						//
						ScaledWindowEx (buffer2, N, FLATTOP, 0, &winConst2);
						AutoPowerSpectrum(buffer2,N,1.0/fs,autoSpectrum2,&df);
						PowerFrequencyEstimate(autoSpectrum2,N/2,-1,winConst2,df,7,&frequencyPeak,&powerPeak);
						SpectrumUnitConversion (autoSpectrum2, N/2, SPECTRUM_POWER, SCALING_MODE_LINEAR, DISPLAY_UNIT_VRMS,df, winConst, spectru, unit);
						//afisare spectru
						DeleteGraphPlot (acqPanel, PANEL_2_GRAPH_3, -1, VAL_IMMEDIATE_DRAW);
						PlotWaveform (acqPanel, PANEL_2_GRAPH_3, spectru2, N/2, VAL_DOUBLE, 1.0, 0.0, 0.0, 1.0, VAL_THIN_LINE, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_RED);
			...
			if(doSpectruF == 1)
			{
				//ferestruirea o facem pe tot intervalul =>eu asa am facut :)
				GetCtrlVal(acqPanel,PANEL_2_RING,&tipFereastra);
				//0 flat top
				//1 welch
				GetCtrlVal(acqPanel,PANEL_2_RING_2,&tipFiltru);
				...
				if (tipFereastra == 0 )
				{
					...
					{
						buffer2[i]=waveData[i];
					}
					//ferestruire
					ScaledWindowEx (buffer2, npoints, FLATTOP, 0, &winConst1);
					//filtrare
					if(tipFiltru == 0)
					{
						Bw_HPF (buffer2, npoints,sampleRate ,1/3*frequencyPeak , 5, buffer2);
					}
					else
					{
						Ch_BPF (buffer2, npoints, sampleRate, (1/4)*frequencyPeak, (1/2)*frequencyPeak, 0.1, 5, buffer2);
					}
				}
				else
				{
					...
					//ferestruire
					ScaledWindowEx (buffer2, npoints, WELCH, 0, &winConst1);	
					//filtrare
				}
			}
			*/